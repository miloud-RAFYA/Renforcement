...
objets = temperatures.map((t) => {
  if (t >= 30) {
    return { celsius: (t * 9) / 5 + 32, statut: "Chaud" };
  } else if (t < 0) {
    return { celsius: (t * 9) / 5 + 32, statut: "froid" };
  } else {
    return { celsius: (t * 9) / 5 + 32, statut: "tempere" };
  }
});
// exer:4
const mots = [
...
const messageUndefined = catalogue.find((c) => c.ref === "A33") || {
  ref: "inconue",
};
...
const animaux = ["chat", "chien", "chat", "oiseau", "chat", "chien"];
...
const mynNotes = someNotes / notes.length;
const maxNote = notes.reduce((acc, n) => (n > acc ? n : acc));
const noteDessusVg = notes.reduce(
  (acc, n) => (n < mynNotes ? acc + 1 : acc),
  0,
);
// exer:7
